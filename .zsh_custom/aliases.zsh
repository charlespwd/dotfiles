...
if [ "$OS" = "Linux" ]; then
  ...
  alias ls="$(brew --prefix)/opt/coreutils/libexec/gnubin/ls --color -h --group-directories-first -l"
  alias pimount="sshfs rpi.local: ~/ws/pi"
  alias piunmount="umount ~/ws/pi"
  alias tvc="open $BIN/toggle-virtual-cam.app"
  alias nginx="/opt/nginx/bin/nginx"
  alias nvim="$(brew --prefix)/bin/nvim"
