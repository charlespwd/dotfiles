...
if [ "$OS" = "Linux" ]; then
  ...
  alias decrypt="gpg"
  alias encrypt="gpg -c"
  alias fgpr="BROWSER=/usr/bin/firefox-developer-edition gpr"
  alias gtypist="gtypist -w"
  alias rtv="BROWSER=/usr/bin/firefox-developer-edition rtv"
  alias mountsp="sudo mount -t cifs //raspi.local/stockpile /mnt/Stockpile -o username=pi,vers=1.0"
  ...
  alias unmountsp="sudo umount /mnt/Stockpile"
elif [ "$OS" = "Darwin" ]; then
...
alias aldo-vpn="sudo openconnect -u cpclermont.cg.ide --authgroup=ALDOGROUP vpn.aldogroup.com"
